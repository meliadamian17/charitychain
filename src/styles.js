const styles = {
    //we can definitely toy around with these, just some simple font styles i wanted to use for most of the site
    paddingX: "sm:px-16 px-6",
    paddingY: "sm:py-16 py-6",
    padding: "sm:px-16 px-6 sm:py-16 py-10",
  
    mainSectionTitleText:
       "font-black text-my-olive lg:text-[60px] sm:text-[60px] xs:text-[50px] text-[40px] lg:leading-[98px] mt-2",
    mainSectionSubText:
      "text-[#dfd9ff]  text-light-teal font-medium lg:text-[25px] sm:text-[26px] xs:text-[20px] text-[16px] lg:leading-[40px]",
  
    subSectionHeadText:
      "text-white font-black md:text-[60px] sm:text-[50px] xs:text-[40px] text-[30px]",
    subSectionSubText:
      "sm:text-[30px] text-[30px] text-light-teal tracking-wider",
    
  };
  
  export { styles };